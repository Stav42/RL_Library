...
class Simulation:
    ...
    def train(self, seed=1):
        ...
        for update in range(1, num_upd+1):
            ...
            global_time.append(time.time()-initial_time)
            table_rollout = [[x, y] for (x, y) in zip(global_step_list, global_time)]
            table_rollout = wandb.Table(data=table_rollout, columns=["rollouts", "time"])
            wandb.log(
                {"Sampling Rollout time": wandb.plot.line(table_rollout, "rollouts", "time", title="Sampling rollout vs time taken")}
            )
