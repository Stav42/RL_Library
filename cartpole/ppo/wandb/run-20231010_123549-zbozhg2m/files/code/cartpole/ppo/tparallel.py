...
class Simulation:
    ...
    def train(self, seed=1):
        ...
        for update in range(1, num_upd+1):
            ...
            print(f"Time after sampling: {time.time()}")
            global_time+=time.time()-update_start   
            global_step_list.append(global_step)
            global_time_list.append(global_time)
            print(f"rollout steps: {global_step} time taken overall {time.time()-update_start} time taken per step: {global_time/global_step}")
            ...
            wandb.log(
                {"Sampling Rollout time per step": wandb.plot.line(table_rollout, "rollouts", "time", title="Sampling time per rollout")}
            )

            self.writer.add_scalar('Sampling rollout vs time', scalar_value=time.time()-update_start, global_step=global_step)
